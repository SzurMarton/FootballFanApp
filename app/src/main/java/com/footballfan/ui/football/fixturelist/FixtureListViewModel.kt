...
class FixtureListViewModel @Inject constructor(
    ...
    fun loadFixturesData(leagueId: Int) = execute {
        ...
        viewState = fixtureList?.let { rounds?.let { it1 -> DataReady(it, it1) } }
                ?: Error
    }

    fun getFilteredFixtures(filter: String?): List<FixtureListUiData>? {
        val list = fixtureList?.fixtures
        return list?.filter { it.leagueRound == filter } ?: emptyList()
    }

    fun getAllFixtures(): List<FixtureListUiData>? {
        return fixtureList?.fixtures
    }
}
