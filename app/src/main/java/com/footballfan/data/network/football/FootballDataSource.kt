...
class FootballDataSource @Inject constructor(private val footballApi: FootballApi){
    ...
    suspend fun getLineUps(fixtureID: String): NetworkResponse<DomainLineUpsData> =
        executeNetworkCall {
            footballApi.getLineUps(fixtureID).response.let { it ->
                DomainLineUpsData(
                    homeCoachName = it?.first()?.coach?.name, //first element in the list is the home team and last is the away team, list size = 2
                    homeFormation = it?.first()?.formation,
                    awayCoachName = it?.last()?.coach?.name,
                    awayFormation = it?.last()?.formation,
                    homeStartXI = it?.first()?.startXI,
                    homeSubstitutes = it?.first()?.substitutes,
                    awayStartXI = it?.last()?.startXI,
                    awaySubstitutes = it?.last()?.substitutes
                )
            }
        }
    ...
    suspend fun getStanding(season: Int, leagueid: Int): NetworkResponse<DomainStandingsData> =
        executeNetworkCall {
            footballApi.getStandings(season, leagueid).response.first()
                .let { it ->  //TODO handle cups and ucl like competitions list in list like response object?
                    DomainStandingsData(
                        leagueID = it.league.id,
                        leagueLogo = it.league.logo,
                        leagueName = it.league.name,
